...
var AzureStreamingFuncTestCmd = &cobra.Command{
	Use:   "azure-streaming-func",
	Short: "Test Azure AI streaming with WithStreamingFunc (backward compatibility)",
	Run:   runAzureStreamingFuncTest,
}

func init() {
	AzureStreamingFuncTestCmd.Flags().String("model", "gpt-4o", "Azure AI model to test")
	AzureStreamingFuncTestCmd.Flags().String("endpoint", "", "Azure AI endpoint (or set AZURE_AI_ENDPOINT env var)")
	AzureStreamingFuncTestCmd.Flags().String("api-key", "", "Azure AI API key (or set AZURE_AI_API_KEY env var)")
}

func runAzureStreamingFuncTest(cmd *cobra.Command, args []string) {
