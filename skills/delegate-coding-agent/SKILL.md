...
| --- | --- | --- |
| Powerful | Ambiguous architecture, cross-module debugging, security-sensitive work, difficult planning, or final review of high-risk changes | Codex `gpt-5.5`; Claude `claude-opus-5`; Pi `google/gemini-3.1-pro-preview` or `zai/glm-5.2` |
| Balanced | Normal feature work, refactors, tool-heavy implementation, bug investigation, and tests | Codex `gpt-5.4`; Claude `claude-sonnet-5`; Cursor `composer-2.5`; Pi `google/gemini-3.7-flash`, `minimax/MiniMax-M2.7`, or `moonshotai/kimi-k2.7-code` |
