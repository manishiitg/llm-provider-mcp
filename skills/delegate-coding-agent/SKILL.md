...
| --- | --- | --- |
| Powerful | Ambiguous architecture, cross-module debugging, security-sensitive work, difficult planning, or final review of high-risk changes | Codex `gpt-6-astra` or `gpt-6-sol`; Claude `claude-opus-5-5`; Pi `google/gemini-3.1-pro-preview` or `zai/glm-5.3` |
| Balanced | Normal feature work, refactors, tool-heavy implementation, bug investigation, and tests | Codex `gpt-6-sol`; Claude `claude-sonnet-5`; Cursor `composer-2.5`; Pi `google/gemini-3.8-flash`, `minimax/MiniMax-M3`, or `moonshotai/kimi-k3` |
