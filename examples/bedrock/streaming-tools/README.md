# Bedrock Streaming with Multi-Tool Calls Example

A comprehensive example demonstrating how to use AWS Bedrock with multiple parallel tool calls and streaming enabled.
...
- Initializes the Bedrock provider with `global.anthropic.claude-sonnet-4-5-20250929-v1:0` model
...
- **Multiple Parallel Tool Calls**: Shows how Bedrock can call multiple tools simultaneously in a single response
...
```
Initializing Bedrock provider with model: global.anthropic.claude-sonnet-4-5-20250929-v1:0 (region: us-east-1)
...
Bedrock (Claude) can call multiple tools in a single response. All tool calls are streamed when complete, allowing you to process them in real-time.
