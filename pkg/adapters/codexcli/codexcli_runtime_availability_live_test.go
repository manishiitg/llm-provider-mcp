...
// TestCodexRuntimeAvailabilityLive is the CertRuntimeAvailability P0 proof
// for Codex: the contract binary resolves on PATH and answers the version
...
func TestCodexRuntimeAvailabilityLive(t *testing.T) {
	if !*codingCLIP0Live {
		t.Skip("run through the live coding CLI P0 runner")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()
	version, err := llmtypes.ProbeCLIBinaryVersion(ctx, "codex", "--version")
	if err != nil {
