...
func TestFindCodexRolloutForSessionScanUsesBindPrompt(t *testing.T) {
	...
	if got := findCodexRolloutForSessionScan(turnStart, workingDir, nil, alphaPrompt); got != alpha {
		t.Fatalf("alpha bound to %q, want its own rollout %q", got, alpha)
	}
	if got := findCodexRolloutForSessionScan(turnStart, workingDir, nil, betaPrompt); got != beta {
		t.Fatalf("beta bound to %q, want %q", got, beta)
	}
	if got := findCodexRolloutForSessionScan(turnStart, workingDir, nil, "a prompt no rollout has recorded yet"); got != "" {
	...
	if got := findCodexRolloutForSessionScan(turnStart, workingDir, nil, longAlpha); got != la {
		t.Fatalf("long alpha bound to %q, want %q", got, la)
	}
	if got := findCodexRolloutForSessionScan(turnStart, workingDir, nil, longBeta); got != lb {
