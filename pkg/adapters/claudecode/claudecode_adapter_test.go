...
func TestClaudeCodeNewModelMetadataIsExplicit(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.modelID, func(t *testing.T) {
			...
			structured := NewClaudeCodeAdapter("", tt.modelID, &MockLogger{})
			meta, err = structured.GetModelMetadata(tt.modelID)
			if err != nil {
				t.Fatalf("structured metadata error: %v", err)
			}
			if meta.ModelName != tt.wantName || meta.InputCostPer1MTokens != tt.wantInput || meta.OutputCostPer1MTokens != tt.wantOutput || meta.ContextWindow != tt.wantContext {
				t.Fatalf("structured metadata = %+v, want name=%q input=%v output=%v context=%d", meta, tt.wantName, tt.wantInput, tt.wantOutput, tt.wantContext)
			}
		})
	}
}
...
func TestMapResponseToContentResponse(t *testing.T) {
	...
	// Verify Usage
	// InputTokens = raw input (100) + cache_read (20) + cache_creation (10) = 130
	if resp.Usage.InputTokens != 130 {
		t.Errorf("Expected 130 input tokens (100 + 20 cache_read + 10 cache_creation), got %d", resp.Usage.InputTokens)
	}
