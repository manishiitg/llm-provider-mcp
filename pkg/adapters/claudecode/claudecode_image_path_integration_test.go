...
// TestClaudeCodeRealImagePathAnalysis proves Claude Code's own native vision
// -- pointed at a local workspace file path, the same mechanism
// wrapReadImageWithLLM (agent_go) uses for read_image(provider="claude-code")
// -- actually works. No prior test in this package covered image
// understanding at all (this package has no other image-related test file).
func TestClaudeCodeRealImagePathAnalysis(t *testing.T) {
...
func writeSolidClaudeTestPNG(t *testing.T, path string, pixel color.RGBA) {
	t.Helper()
